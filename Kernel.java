	...
	case INTERRUPT_SOFTWARE: // System calls
    	    ...
    	    case WRITE:
        		switch ( param ) {
                    ...
                    default:
                        ...
                        // retrieve tcb from scheduler 
                        myTcb = scheduler.getMyTcb();

                        // retrieved tcb is there
                        if (myTcb != null) {

                            // retrieve corresponding file table entry from file table
                            FileTableEntry entry = myTcb.getFtEnt(param);

                            // if file table entry is there
                            if (entry != null) {

                                // write to the entry by calling corresponding filesystem func
                                return fs.write(entry, (byte[])args);
                            }
                        }
                        // if anything was null above, return error notice code of -1
                		return ERROR;
        		}
	    ...
	    case OPEN:    
            ...
            if (myTcb != null) {
                ...
                FileTableEntry entry = fs.open(s[0], s[1]);
	    ...
	    case SEEK:    // to be implemented in project
            ...
            if (myTcb != null) {
                ...
                FileTableEntry entry = myTcb.getFtEnt(param);
                if (entry != null) {
                    return fs.seek(entry, args1[0], args1[1]);
                }
            }
            return ERROR;
	    ...
	    case DELETE:  // to be implemented in project
            return fs.delete((String)args) ? 0 : -1;
	    }
	    return ERROR;
